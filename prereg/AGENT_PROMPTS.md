...
Amendment 11 — 2026-09-18. Batch 5: config s3 under the recalibrated σ =
194.406790 (amendment 9), {control, gate} × claude-fable-5-1, 40 runs per arm,
80 runs, seeds 581–660, arms strictly alternating. Thinking is disabled, as §3
pins and amendment 7 already applied to this model; the model string is checked
per call. The per-run seed draw is extended from 581 to 661 values; §4's
construction is unchanged, every earlier draw is byte-identical, and no seed in
581–660 repeats one already drawn. The schedule is
experiments/schedule_80_s3_fable.txt, generated by experiments/make_schedule.py
rather than written by hand. Comparison with batch 4's Sonnet cells at the same
σ is exploratory and carries no error control: PASS rate, deflation gap,
