...
degeneracy-calibration chose support_min=5, tau=0.001 by its pre-registered rule, but its ground
...
seeds so the new definition is not fitted to degeneracy-calibration's data, and enlarges the
populations so the detection target is not 23 cases.

Changes from degeneracy-calibration, all fixed before running
  ...
  Populations, fresh seeds: D1 3,000, D2 1,500, D3 3,000, D4 1,500 (9,000
    dense); S1 1,000, S2 1,000, S3 200 (same constructions as degeneracy-calibration).
...
Usage: python -m experiments.degeneracy_recalibration [--workers N] [--smoke]
"""
...
from experiments import degeneracy_calibration as calib
...
OUTPUT = "figures/degeneracy_recalibration_data.pkl"
...
def build(name: str, seed: int):
    if name == "D1":
        return calib.dense_ma("null_grid", seed)
    if name == "D2":
        return calib.dense_ma("real_edge", seed)
    if name == "D3":
        return calib.gridsearch(seed)
    if name == "D4":
        return calib.dense_ma("null_grid", seed, t_df=3)
    if name == "S1":
        return calib.sparse_band(calib.S1_RULES, 2520, seed)
    if name == "S2":
        return calib.sparse_band(calib.S2_RULES, 1000, seed)
    return calib.sparse_band(calib.S3_RULES, 1000, seed)
...
def diagnose(name: str, seed: int) -> dict:
    ...
    nz = (R != 0).astype(float)
    sr_sel = float(sharpe(R[:, [j]], annualization=calib.ANN)[0])
    ...
    for start in range(0, B, CHUNK):
        ...
        sr = np.where(live, m1 / np.sqrt(np.where(live, var, 1.0)), 0.0) * calib.ANN
        cap_binds += int(np.sum(np.abs(sr) > SHARPE_CAP))
        sr = np.clip(sr, -SHARPE_CAP, SHARPE_CAP)
        sr_fixed = np.where(sd_full[:, None] > 0, m1 / safe_sd[:, None], 0.0) * calib.ANN
    ...
    base = calib.verdict(M, tracked, sr_sel)
    top = M >= np.quantile(M, 1 - TOP_FRACTION)
    return {
        "population": name, "seed": seed, "N": N, "T": T, "base": base,
        "fixed": calib.verdict(M_fixed, tracked_fixed, sr_sel),
        ...
        "candidates": {
            cand: {"all": float(flagged_arg[cand].mean()), "top10": float(flagged_arg[cand][top].mean()),
                   "flip": calib.verdict(M_excl[cand], tracked, sr_sel) != base}
