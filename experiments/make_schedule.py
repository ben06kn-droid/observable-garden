"""Generate the batch schedules run_arms.sh consumes.
    ...
    python -m experiments.make_schedule --batch 5  > experiments/schedule_80_s3_opus.txt
    python -m experiments.make_schedule --batch 5 --model claude-fable-5-1 \
                                                   > experiments/schedule_80_s3_fable.txt
...
batch 5 is amendment 11's second model at that same sigma -- Opus now, Fable
deferred to the allowance reset (amendment 12) on the same seeds, so the two are
paired by DGP draw and both stay regenerable from here.
...
# Amendment 12: Opus runs these seeds now, Fable runs the same ones after the
# allowance resets. One model per emitted schedule, both regenerable from here;
# the run id carries the model tag, so the two never share a directory.
B5_MODELS = ("claude-opus-5", "claude-fable-5-1")
B5_MODEL = B5_MODELS[0]
B4_SEEDS = range(B4_FIRST_SEED, B4_FIRST_SEED + len(B4_ARMS) * B4_PER_ARM)


def build_batch5(model: str = B5_MODEL) -> list[tuple[int, str, str, int | None, str]]:
    """Strict alternation, as §4 requires of a two-arm batch."""
    if model not in B5_MODELS:
        raise SystemExit(f"batch 5 model must be one of {B5_MODELS}, got {model!r}")
    rows = []
    seed = B5_FIRST_SEED
    for _ in range(B5_PER_ARM):
        for arm in B5_ARMS:
            rows.append((seed, "s3", arm, None, model))
            seed += 1
    return rows


def verify_batch5(rows, model: str = B5_MODEL) -> None:
    got = Counter(a for _, _, a, _, _ in rows)
    for arm in B5_ARMS:
    ...
    if {m for _, _, _, _, m in rows} != {model}:
        raise SystemExit(f"this batch 5 schedule is {model} only")
...
def main() -> None:
    ...
    ap.add_argument("--model", choices=B5_MODELS, default=B5_MODEL,
                    help="batch 5 only: which model's schedule to emit (amendment 12). "
                         "Both draw the same seeds, so the two are paired by DGP draw.")
    a = ap.parse_args()

    print("# seed  config  arm      budget  model")
    print("# Generated by experiments/make_schedule.py; regenerate rather than edit.")
    if a.batch == "5":
        rows5 = build_batch5(a.model)
        verify_batch5(rows5, a.model)
        print(f"# batch 5, seeds {B5_FIRST_SEED}-{B5_FIRST_SEED + len(rows5) - 1}: s3 "
              f"{B5_ARMS}, {B5_PER_ARM} per arm, {len(rows5)} rows, {a.model} at the "
              f"recalibrated s3 sigma (prereg §6, amendments 11 and 12). The other "
              f"model in B5_MODELS runs these same seeds, paired by DGP draw.")
