"""E18b: does the naive scoring rule explain the gap between E18's and e4's Adaptive type-I?

Pre-registered in prereg/E18b.md, committed before this ran. E18's adaptive row, same draws, block length
and resampled indices; the only change is the Sharpe the naive null is compared against: Adaptive's own
selected value (E18's rule) or the largest Sharpe in the transcript (e4's rule, deflate's default).

Usage: python -m experiments.e18b_scoring_rule [--workers N]
"""
...
from experiments.e18_depth import ALPHA, B, BLOCK, K, M, N_DOSE, RHO, SEED0, T, T_OOS, git_state
from searchers.scripted import Adaptive

E18_OUTPUT = "figures/e18_depth_data.pkl"
OUTPUT = "figures/e18b_scoring_rule_data.pkl"
...
def report(data: dict) -> None:
    ...
    print(f"\nE18's adaptive row, n={N_DOSE}, B={B}; git at launch {git['commit'][:7]}{' (dirty)' if git['dirty'] else ''}")
    with open(E18_OUTPUT, "rb") as f:
        e18 = pickle.load(f)["adaptive"]["p_naive"]
    reproduced = np.array_equal(data["p_selected"], e18)
    print(f"  1. reproduction: p under E18's rule equals E18's stored p_naive on all draws: {reproduced}")
    if not reproduced:
        print("     STOP: not E18's draws; nothing below is interpreted.")
        return
    ...
    for field, label in (("p_selected", "selected value (E18)"), ("p_transcript_max", "transcript maximum (e4)")):
